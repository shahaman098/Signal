...
This file is the source of truth for whether Signal is ready for Devpost submission.

Fill the required fields with real public links before submitting. Do not use `localhost`, private IPs, internal-only URLs, or invented values.
...
These fields can improve the judging story, but they are not required by the live Devpost minimum.

- Alibaba Cloud API base URL: `https://signal-en-agent-ersgaojhti.ap-southeast-1.fcapp.run`
- Verified health URL: `https://signal-en-agent-ersgaojhti.ap-southeast-1.fcapp.run/health`
- Container image reference: `OPTIONAL_NOT_REQUIRED`
- Model Studio workspace ID: `ws-sa6mvuhwolukydvg`
- Managed Agent ID: `OPTIONAL_NOT_REQUIRED`
- Remote MCP service URL or identifier: `OPTIONAL_NOT_REQUIRED`
- Agent environment ID: `OPTIONAL_NOT_REQUIRED`
- Verified session ID: `OPTIONAL_NOT_REQUIRED`
- Verified session output note: `2026-07-15: Function Compute root UI returned hosted HTML; health returned HTTP 200; /api/creative/autopilot and /api/creative/radar returned mode=live, provider=Alibaba Cloud Model Studio, model=qwen-plus, status=pending_human_review. See docs/proof/function-compute-live-proof.md.`
